...
#include "GFX_startup.h"

STATIC CONST TEXT __TEXTSEGMENT__ verstring[] = VERSTAG;
STATIC CONST TEXT libname[] = "sdl2_gfx.library";

struct ExecBase   *SysBase  = NULL;
struct DosLibrary *DOSBase  = NULL;
struct Library    	*SDL2Base = NULL;
struct Library 		*SDL2GfxBase = NULL;
...
STATIC VOID sort_ctdt(struct SDL2GfxLibrary *LibBase)
{
...
static int init_libs(struct SDL2GfxLibrary *base, struct ExecBase *SysBase)
{
...
struct Library	*LIB_Init(struct SDL2GfxLibrary *LibBase, BPTR SegList, struct ExecBase *sysBase)
{
	register char *r13;
...
static BPTR DeleteLib(struct SDL2GfxLibrary *LibBase, struct ExecBase *SysBase)
{
	BPTR SegList = 0;
...
static void UserLibClose(struct SDL2GfxLibrary *LibBase, struct ExecBase *SysBase)
{
...
{
	struct SDL2GfxLibrary *LibBase = (APTR)REG_A6;
...
BPTR LIB_Close(void)
{
	...
	if (LibBase->Parent)
	{
		struct SDL2GfxLibrary *ChildBase = LibBase;
...
{
	struct SDL2GfxLibrary *LibBase = (APTR)REG_A6;
	struct SDL2GfxLibrary	*newbase, *childbase;
	struct ExecBase *SysBase = LibBase->MySysBase;
	...
	ForeachNode(&LibBase->TaskContext.TaskList, ChildNode)
	{
		...
		{
			/* Yep, return it */
			childbase = (APTR)(((ULONG)ChildNode) - offsetof(struct SDL2GfxLibrary, TaskContext.TaskNode.Node));
